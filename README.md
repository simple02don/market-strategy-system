...
`tail-review` 不读取真实账户，只管理系统模拟持仓。它优先复评前夜正式候选，同时允许热榜前列中满足上市时长、市值、20 日平均成交额和尾盘结构要求的新热门股进入“尾盘热榜规则发现”通道。该通道不会伪造夜间模型概率，并会在推送中与“夜间正式候选复评”明确区分。对模拟持仓输出继续持有、转弱观察、退出或 T+1 锁定次日退出；尾盘新入场会直接记为系统模拟成交。
