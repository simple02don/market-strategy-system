...
def _candidate_card(candidate: dict[str, Any]) -> str:
    ...
    risks = intent.get("risks") or candidate.get("valuation_risk") or "按系统止损与放弃条件执行"
    return f"""
    <article class="stock-card">
      ...
      <div class="metrics"><div><b>{_num(candidate.get('score'), 1)}</b><span>综合评分</span></div><div><b>{_pct(probability)}</b><span>上涨概率</span></div><div><b>{_pct(premium.get('factor_coverage'))}</b><span>因子覆盖</span></div><div><b>{_pct(intent.get('one_day_risk'))}</b><span>一日游风险</span></div></div>
      <dl>
...
def generate_report(payload: dict[str, Any], output: Path) -> Path:
    ...
    sector_rows = "".join(
        f"<tr><td>{_esc(item.get('industry'))}</td><td>{_esc(item.get('role'))}</td><td>{_num(item.get('score'), 1)}</td><td>{_num(item.get('ret1'), 2)}%</td><td>{_num(item.get('excess_20d'), 2)}%</td></tr>"
    ...
    evidence_cards = "".join(
        f"<article class='evidence'><span>{_esc(_plain_text(item.get('source')))} · {_esc(item.get('published_at') or item.get('publish_time'))}</span><h3>{_esc(_truncate(item.get('title'), 100))}</h3><p>{_esc(_truncate(item.get('impact_reason') or item.get('reason') or item.get('summary')))}</p></article>"
        for item in evidence_items[:8]
    ) or "<p class='empty'>有效新闻与政策证据不足。</p>"
    hypotheses = "".join(
        f"<li><b>{_esc(item.get('name'))}</b>：{_esc(item.get('reason') or item.get('rationale'))}</li>"
...
</style></head><body><main class="wrap"><header><h1>主力策略情景推演</h1><div class="meta">数据截止 {trade_date} · 目标交易日 {next_day} · {run_mode}</div></header>
...
<section class="card"><h2>资金意图与市场宽度</h2><p><b>下一阶段：</b>{_esc(intent_forecast.get('label'))}　<b>目标板块：</b>{_esc('、'.join(target_sectors) or '防守/等待')}</p><p>上涨 {_esc(breadth.get('up'))} / 下跌 {_esc(breadth.get('down'))} · 涨停 {_esc(breadth.get('limit_up'))} / 跌停 {_esc(breadth.get('limit_down'))} · 60日新高 {_esc(breadth.get('new_high_60d'))} / 新低 {_esc(breadth.get('new_low_60d'))}</p><h3>操盘行为假设</h3><ul>{hypotheses}</ul></section>
...
<section class="card"><h2>次日新推荐</h2><p class="meta">最多5支；股票名称即唯一展示标识，具体代码仅保留在系统内部。早确认只对高质量候选开放，其余仍等待15分钟标准确认。</p><div class="stock-grid">{candidate_cards}</div></section>
...
<section class="card"><h2>新闻 / 政策 / 情绪证据</h2><p>市场情绪 {_esc(evidence.get('market_sentiment'))} · 证据置信度 {_pct(evidence.get('confidence'))} · 来源覆盖 {_pct(evidence.get('coverage'))} · 有效资讯 {_esc(evidence.get('valid_items'))} 条</p>{evidence_cards}</section>
